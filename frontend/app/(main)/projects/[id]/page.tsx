...
export default function ProjectDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: project, error: projectError, isLoading: projectLoading } = useSWR(`/api/projects/${id}`, fetcher);
  
  // Fetch Project Sandboxes
  const { data: environments, error: envError, isLoading: envLoading } = useSWR<Environment[]>(`/api/environments?projectId=${id}`, fetcher, {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-on-surface-variant mt-1">
              ...
              <span>{isDefaultWorkspace ? "Private Workspace" : "Team Workspace"}</span>
              {project.description && (
                <>
      ...
      <div className="space-y-4">
        ...
        {envLoading ? (
          ...
          <div className="bg-surface-container-lowest border border-outline-variant border-dashed rounded-xl py-16 text-center flex flex-col items-center">
            ...
            <h3 className="text-lg font-bold text-on-surface mb-2">No sandboxes found</h3>
            <p className="text-on-surface-variant text-sm mb-6 max-w-xs">There are no sandboxes in this project yet.</p>
            <Link href="/upload" className="bg-primary-container text-on-primary-fixed-variant px-6 py-2.5 rounded-xl font-bold text-sm hover:shadow-[0_0_20px_rgba(0,240,255,0.2)] active:scale-95 transition-all">
              New Sandbox
            </Link>
          </div>
        ) : (
